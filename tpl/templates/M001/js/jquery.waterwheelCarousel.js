    ...
    function preload(callback) {
      ...
      $imageElements.each(function () {
        $(this).bind('load', function () {
          ...
          loadedImages += 1;
		  var tit = $(this).attr("bname");
		  if($(this).parent().find('div').size()){}else{
		  $(this).after('<div class="txt">'+tit+'</div>');
		  }
    ...
    function moveItem($item, newPosition) {
      ...
      if (Math.abs(newPosition) <= options.flankingItems + 1) {
        ...
        $item
		  ...
		  $item.next('div').css({'z-index':$item.data().depth
		  }).animate({
            top:     $item.data().top+$item.data().height+16,
			 left:    $item.data().left,
            width:   $item.data().width,
            opacity: $item.data().opacity
          }, data.currentSpeed);
